...
contract Pausable is Ownable {
    ...
    function setPaused(bool pause) public onlyOwner {
        _paused = pause;
        if (pause) emit Paused(msg.sender);
    ...
    modifier whenNotPaused() {
        require(!_paused, "contract mustn't be paused");
        _;
    }
    ...
    event Paused(address adr);
    event Unpaused(address adr);
}
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId], "given address mustn't be the owner's");

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == owner() || isApprovedForAll(owner(), msg.sender), 'need to be contract owner or approved for all');
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        require(to == address(to), 'given address is invalid');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal virtual {
        ...
        require(from == _tokenOwner[tokenId], 'must be token owner');

        // require token is being transferred to valid address
        require(to == address(to), 'must be valid address');
...
contract ERC721Metadata is ERC721Enumerable {
    ...
    constructor (string memory __name, string memory __symbol, string memory __baseTokenURI) {
    ...
    function setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId), 'tokenId does not exist');
        string memory str = uint2str(tokenId);
        _tokenURIs[tokenId] = string(abi.encodePacked(_baseTokenURI, str));
    }
...
contract MyLittleHouseToken is ERC721Metadata {

    //  1) Pass in appropriate values for the inherited ERC721Metadata contract
    //      - make the base token uri: https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/
    constructor() ERC721Metadata('MyLittleHouseToken', 'MLH', 'https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/') { }
